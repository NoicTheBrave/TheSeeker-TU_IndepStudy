...
def on_press(key):
    ...
    if k in ['down', 'up', 'left', 'right', 'space']:  # keys of interest
        ...
        if k == 'up':#FORWARDS
            msg = "S160" #S = Speed, Number = PWM Fed to the ESC, starting @ ~150 -> 255
            ...
            msg =  "T50"           #T = Turn, Number = Angle (0->180, however ~50 -> ~130 might be best USEFUL range...?)
            serial1.write(msg.encode())
            print("LEFT") 
        elif k == 'right': #TURN RIGHT
            msg =  "T130" #PLACE HOLDER          
            serial1.write(msg.encode())
            print("RIGHT")
